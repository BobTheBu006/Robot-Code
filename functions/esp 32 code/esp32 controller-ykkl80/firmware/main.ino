...
bool handleStepZCommand(const String &cmd) {
  ...
  bool rightBlocked = false;
  bool stopRequested = false;
  // Report how far the axes actually got. The host drives switch-seeking
  // moves by commanding the whole travel and sending STOP the moment its own
  // GPIO sees the switch trip, so the commanded count is not the distance
  // travelled - only these counters are.
  long beforeLeft = zLeftAxis.currentSteps;
  long beforeRight = zRightAxis.currentSteps;
  ...
  long doneLeft = zLeftAxis.currentSteps - beforeLeft;
  long doneRight = zRightAxis.currentSteps - beforeRight;

  (void)completed;
  Serial.print(stopRequested ? "OK STOP STEP Z " : "OK STEP Z ");
  Serial.print(doneLeft);
  Serial.print(" ");
