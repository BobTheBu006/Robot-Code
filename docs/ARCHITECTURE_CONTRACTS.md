...
- hardware canvas block positions for operators' saved layout
...
Controller, device, and group IDs must be unique across the Hardware Map. Controller IDs are internal stable references generated by the system and should not be edited by operators; users change the USB port binding instead. Disabled controllers must not reserve a USB port for active controller selection.
